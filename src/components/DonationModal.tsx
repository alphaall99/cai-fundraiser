...
export default function DonationModal({ block, open, onClose, onDonate }: DonationModalProps) {
  ...
  const handleProceed = () => {
    ...
    if (!amount || amount <= 0 || amount > remaining) {
      toast.error(`Veuillez entrer un montant entre 1 $ et ${remaining.toLocaleString()} $`);
      return;
    }
    setStep("payment");
  };
  ...
  const handleDonate = () => {
    ...
    toast.success("Merci pour votre don généreux ! 🕌", {
      description: `${amount.toLocaleString()} $ donné au bloc ${tier.label}`,
    });
    resetAndClose();
  };
      ...
      <DialogContent className="sm:max-w-md border-border/50 shadow-card">
        <DialogHeader>
          <DialogTitle className="font-display text-xl flex items-center gap-2">
            ...
            Bloc {tier.label} — {tier.price.toLocaleString()} $
          </DialogTitle>
          <DialogDescription>
            {block.donated > 0
              ? `${block.donated.toLocaleString()} $ déjà financés. ${remaining.toLocaleString()} $ restants.`
              : "Soyez le premier à contribuer à ce bloc !"}
          </DialogDescription>
        </DialogHeader>
        ...
        <AnimatePresence mode="wait">
          {step === "info" ? (
            ...
            >
              ...
              <div className="space-y-2">
                <Label htmlFor="donor-label">Libellé affiché (optionnel)</Label>
                <Input
                  id="donor-label"
                  placeholder="ex. La famille Tremblay"
                  value={label}
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="amount"
                  type="number"
                  placeholder={`Jusqu'à ${remaining.toLocaleString()} $`}
              ...
              <div className="flex items-start gap-2 bg-muted/50 rounded-lg p-3">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Votre don est <strong className="text-foreground">déductible d'impôts</strong> (50 % retourné).
                  Paiement traité par <strong className="text-foreground">Zeffy — 0 % de commission</strong>, 100 % va au centre.
                </p>
              </div>
            ...
            >
              ...
              <p className="text-xs text-muted-foreground text-center">
                Ceci est une démo. En production, vous seriez redirigé vers Zeffy pour le paiement sécurisé.
              </p>
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleDonate} className="flex-1" size="lg">
                  ...
                  Donner {Number(donationAmount).toLocaleString()} $
                </Button>
              </div>
            </motion.div>
          )}
